...
class Cell:
    ...
    def draw(self, canvas, graph):
        ...
        canvas.create_rectangle(self.x*width,self.y*height,(self.x+1)*width, (self.y+1)*height, fill="#FFF" if self.state == ALIVE else "#333")
...
canvasSize = (500,500)
canvas = Canvas(window, width=canvasSize[0], height=canvasSize[1], background="#121212")
canvas.pack()
...
def drawGraph(canvas: Canvas, graph):
    ...
    for i,row in enumerate(graph):
        for j,cell in enumerate(graph[i]):
            canvas.create_rectangle(i*width,j*height,(i+1)*width, (j+1)*height, fill="#FFF" if cell.state == ALIVE else "#333")
