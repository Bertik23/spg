    ...
    def logika(self):
        ...
        def pathFind(start, r = 0):
            ...
            for q in queue:
                ...
                if q[2] >= POCET*2:
                    return None, []
                if sit[q[0]][q[1]].cislo == -1 and q[:2] != (start[0],start[1]):
                    newEnd = pathFind(q[:2], r=r+1)[0]
                    #print(newEnd)
                    end = q
                    if (newEnd != None and r == 0) or (r > 0):
                        return end, values
...
hadi = [Had(random.randint(0,POCET-1), random.randint(0,POCET-1), delka=5, smer=random.randint(0,4), barva=barvy[i], jmeno="", hrac=False) for i in range(5)]
