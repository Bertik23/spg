        ...
        def getSmer(x,y, node):
            ...
            if y == node[1]:
                if x == 0 and node[0] == POCET-1:
                    ...
                    return 2
        queue = PriorityQueue()
        queue.put((0,(self.x,self.y, 0, False, ())))
        values = []
        end = None
            ...
            for x in range(q[0]-1, q[0]+2):
                for y in range(q[1]-1, q[1]+2):
                    ...
                    if sit[x][y].cislo <= 0 and values[x][y] > q[2]+1:
                        ...
                        # if sit[x][y].cislo == -1:
                        #     print("Drobek")
                        values[x][y] = q[2]+1 if not q[4] else q[2]+10
                        #platno.itemconfig(sit[x][y].text, text=str(q[2]+1))
                        queue.put_nowait((q[2]+1 if not q[4] else q[2]+10,(x,y, q[2]+1 if not q[4] else q[2]+10, q[3], q[4])))
            queue.task_done()
        print(a)
        # print("pathFound")
